...
class PostTestCase(TestCase):
    ...
    def test_create_post(self):
        valid_post_data = {"text": "This is a test post.", "origin": None}
    ...
    def test_invalid_post_exception(self):
        invalid_post_data = {"texto": "This is a test post.", "origin": None}
        response = self.client.post(
            reverse("post"),
    ...
    def test_delete_post(self):
        response = self.client.delete(
            reverse("post-detail", args=[self.post.pk]), **authenticate(self.user)
        )
    ...
    def test_delete_post_unauthorized_user(self):
        ...
        response = self.client.delete(
            reverse("post-detail", args=[self.post.pk]), **authenticate(alt_user)
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertTrue(Post.objects.filter(pk=self.post.pk).exists())

    def test_create_post_comment(self):
        valid_comment_data = {"origin": self.post.pk, "text": "This is a test comment."}
        response = self.client.post(
            reverse("post"),
...
class PostActionTestCase(TestCase):
    ...
    def test_get_post_like_count(self):
        ...
        response = self.client.get(
            reverse("post-action", args=[self.post.pk]), **authenticate(self.user)
        )
...
class FeedTestCase(TestCase):
    ...
    def test_get_feed_posts(self):
        response = self.client.get(reverse("feed"), **authenticate(self.user1))
